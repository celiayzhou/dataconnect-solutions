...
from watercooler_utils.ad_ops import get_service_principal_object_id
...
def provision_databricks_cluster(install_config: InstallConfiguration,
                                 ...
                                 wc_sp_secret_value: str,
                                 managed_libraries: list = None,
                                 wc_sp_secret_name: str = "wc-service-principal-secret",
                                 secret_scope_name: str = "jwc",
                                 adb_cluster_name: str = "watercooler-default-cluster",
                                 max_worker: int = 2,
    ...
    :param wc_sp_secret_value:
    :param wc_sp_secret_name:
    ...
    adb_client.secret.put_secret(scope=secret_scope_name, key=wc_sp_secret_name, string_value=wc_sp_secret_value)

    adb_api_token = adb_client.token.create_token(comment="Watercooler Pipeline API token")
    cluster_id = None
...
def create_main_arm_parameters(install_config: InstallConfiguration, base_uri: str = None, sas_token: str = None,
    ...
    if install_config.jwc_aad_app and "appId" in install_config.jwc_aad_app:
        parameters["appservice.aad.clientId"] = {
            "value": install_config.jwc_aad_app["appId"]
        }
        parameters["appservice.aad.clientSecret"] = {
            "value": install_config.jwc_aad_app["password"]
        }
    if install_config.wc_service_principal and "appId" in install_config.wc_service_principal:
        parameters["wc-service.sp.clientId"] = {
            "value": install_config.wc_service_principal["appId"]
        }
        if "wc-service.sp.clientSecret" in install_config.arm_params:
            parameters["wc-service.sp.clientSecret"] = {
                "value": install_config.wc_service_principal["password"]
            }
        if "wc-service.sp.objectId" in install_config.arm_params:
            parameters["wc-service.sp.objectId"] = {
                "value": get_service_principal_object_id(app_id=install_config.wc_service_principal["appId"])
            }
    if install_config.wc_admin_ad_group and "objectId" in install_config.wc_admin_ad_group:
        ...
        parameters["sqlsever.database.aad.admin-login"] = {
            "value": install_config.wc_admin_ad_group["ad_group_name"]
        }
    ...
    parameters['pywc_utils_library.name'] = {
        # "value": install_config.get_pywc_utils_library_name()
        "value": "TODO"
    }
...
def create_sql_init_arm_parameters(install_config: InstallConfiguration, base_uri: str, sas_token: str = None):
    ...
    if install_config.wc_service_principal and "name" in install_config.wc_service_principal:
        parameters['wc-service.sp.name'] = {
            "value": install_config.wc_service_principal['name']
        }

    if install_config.wc_deployer_identity:
        parameters['userIdentityName'] = {
            "value": install_config.wc_deployer_identity['name']
        }
